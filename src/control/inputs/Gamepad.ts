...
export class PGamepadInput extends Input {
    ...
    for (let i = 0; i < this.gamepads.length; i++) {
      ...
      if (gamepad) {
        if (!this.previousButtons[i]) {
          this.previousButtons[i] = new Array(gamepad.buttons.length).fill(
            false,
          );
        }
  ...
  private checkGamepadState() {
    for (let gamepadIdx = 0; gamepadIdx < this.gamepads.length; gamepadIdx++) {
      const gamepad = this.gamepads[gamepadIdx];
      if (!gamepad) continue;
      ...
      for (let buttonIdx = 0; buttonIdx < gamepad.buttons.length; buttonIdx++) {
        ...
        const wasPressed = this.previousButtons[gamepadIdx][buttonIdx];

        if (pressed !== wasPressed) {
          const buttonName = this.getButtonName(buttonIdx);
          this.out(
            InputType.Button,
            buttonName,
            pressed ? ButtonState.PRESSED : ButtonState.UNPRESSED,
          );
          this.previousButtons[gamepadIdx][buttonIdx] = pressed;
        }
      }
...

      {
        ...
        this.out(InputType.Direction, leftStickX, leftStickY, 'leftStick');
      }

      this.previousAxes[gamepadIdx][0] = leftX;
      this.previousAxes[gamepadIdx][1] = leftY;

      // Right stick (axes 2, 3)
      const rightX = gamepad.axes[2];
      const rightY = gamepad.axes[3];
      const prevRightX = this.previousAxes[gamepadIdx][2] || 0;
      const prevRightY = this.previousAxes[gamepadIdx][3] || 0;

      if (
        Math.abs(rightX - prevRightX) > stickDeadzone ||
        ...
        this.out(InputType.Direction, rightStickX, rightStickY, 'rightStick');
      }

      this.previousAxes[gamepadIdx][2] = rightX;
      this.previousAxes[gamepadIdx][3] = rightY;
    }
  }
