...
import { AnimationData, TagData } from '../resources/animation/AnimationData';
...
export class Debugger extends System {
  ...
  public static debugLines: (() => string)[] = [
    ...
    () => 'SFR: ' + Peek.smoothFrameRate.toFixed(1),
    () => 'Focused: ' + Debugger.focusedNodes.size,
    () => `Usage: ${Math.round(Debugger.usagePercent * 100)}%`,
    () => `DefT: ${(Peek as any).deferredTasks.length}`,
  ...
  public constructor(parent: SystemHolder) {
    ...
    // Debugger.runBefore(Vec2, 'addVecWithScalar', function(v, s) {
    //   if (
    //     Number.isNaN(v.x) || Number.isNaN(v.y) ||
    //     Number.isNaN(this.x) || Number.isNaN(this.y)
    //   ) {
    //     Debugger.debugLog('Adding NaN!');
    //     debugger;
    //   }
    //   return { override: false };
    // });
    ...
    Debugger.runBefore(AnimationData, 'getTagData', function (tag) {
      if (!(this as any).tags.has(tag)) {
        Debugger.debugLog(
          "Tag doesn't exist in AnimationData! Existing tags are:",
          (this as any).tags,
        );
        return {
          override: true,
    ...
    Debugger.runBefore(Controller, 'triggerButton', function (_name, state) {
      if (
    ...
    // TODO: why before loading a scene? is that important?
    // Debugger.runBefore(Scene, 'enableSystems', function() {
    ...
    Debugger.runBefore(
      ...
      function () {
        if (
          Debugger.framesLag / Debugger.framesNormal <
          Debugger.lagFrameRatio
        ) {
          Debugger.framesLag++;
    ...
    const hitboxDrawFn = (child: PNode, nest: number) => {
      ...
      // Draw this hitbox
      const hb = child.getHitbox(Peek.snapToGrid);
      hb.x += Peek.frameXOffset / Peek.getPixelScale();
      hb.y += Peek.frameYOffset / Peek.getPixelScale();
      let zeroSize = false;
    ...
    Debugger.runAfter(
      ...
      function () {
        ...
        Debugger.usagePercent = (frameEndTime - frameStartTime) / (1000 / 60);
        ...
        (this as unknown as { ctx: CanvasRenderingContext2D }).ctx.scale(
          scale,
          scale,
        );
        ...
        if (Debugger.showFramerate) {
          const frtHeight = Debugger.frameRateTexture.getHeight() - 1;

          const frameRatePos = Debugger.frameRateToTextureY(Peek.frameRate);
          const frameRatePosSmooth = Debugger.frameRateToTextureY(
            Peek.smoothFrameRate,
          );
          ...
          Debugger.frameRateTexture.setPixel(
            Debugger.frameRateDrawPosition,
            Math.max(frtHeight - ~~(Debugger.usagePercent * frtHeight), 0),
            Color.BLUE,
          );
          Debugger.frameRateTexture.setPixel(
            Debugger.frameRateDrawPosition,
          ...
          if (
            Debugger.frameRateDrawPosition >=
            Debugger.frameRateTexture.getWidth()
          ) {
          ...
          Debugger.frameRateTexture.clearRect(
            ...
            0,
            1,
            Debugger.frameRateTexture.getHeight(),
          );
        ...
        if (Debugger.showAtlas) {
          ...
          for (const atlasRect of TextureAtlas.usedRects) {
            this.rect(
              ~~(atlasRect.x * atlasScale),
              ~~(atlasRect.y * atlasScale),
              ~~(atlasRect.w * atlasScale),
              ~~(atlasRect.h * atlasScale),
              Color.RED,
            );
          }
    ...
    Debugger.controller.onPress('hitboxes', () => {
      ...
      switch (Debugger.showHitboxes) {
        case ShowHitboxLevel.NONE:
          level = ShowHitboxLevel.PHYSICS;
          break;
        case ShowHitboxLevel.PHYSICS:
          level = ShowHitboxLevel.ALL;
          break;
        case ShowHitboxLevel.ALL:
          level = ShowHitboxLevel.NONE;
          break;
      }
    ...
    Debugger.controller.onRelease('pointer', () => {
      ...
      if (!Debugger.isPaused || dragLength > 1) {
        return;
      }
      ...
      const worldPos = scene
        ?.getCamera()
        ?.screenToWorldVec(Debugger.controller.pointer);
  ...
  private static frameRateToTextureY(frameRate: number) {
    let frameRatePos =
      ((120 - frameRate) * Debugger.frameRateTexture.getHeight()) / 120;
    if (frameRatePos < 0) {
      frameRatePos = 0;
  ...
  public static togglePause() {
    if (Debugger.focusedNodes.size === 0) {
      ...
      const cameraCenter = Peek.getScene()?.getCamera()?.getCameraPos();
      if (cameraCenter) {
